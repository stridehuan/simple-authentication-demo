...
##### 一些设计思路说明
1. 各主要模块在内存中是存在形式是单例对象，通过一个Application实例包装，作为容器，这么设计的原因是：
   - 单例是为了安全，防止初始化多份实体
   - 以对象的形式而不是类来作为实体，是为了统一灵活的管理，比如将门面对象放到Application容器中管理方便通过动态代理对实体进行统一增强处理
2. 为什么使用业务线程池
   - 引入业务线程池来执行业务逻辑是为了更好地还原实际服务的场景，本项目虽然可以不做业务线程池，直接使用客户端线程执行业务逻辑，但是引入业务线程池后client和server是两个独立的实体，模型上会更加完整和合理
   - 本次项目中的客户端是同一个进程下的线程，没有网络IO，如果改用通过Socket的方式访问接口，就需要使用NIO和Reactor模型来将业务交给业务线程池处理了，所以引入业务线程池的设计更符合一个独立服务的场景
